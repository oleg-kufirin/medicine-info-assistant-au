...
REFUSAL_EMPTY = (
    "The query is empty. Please ask a question about a medicine that can be answered using official Consumer Medicine Information or Product Information documents."
)
...
REFUSAL_UNSUPPORTED = (
    "I can only provide general information that appears in official Consumer Medicine Information or Product Information documents."
)
...
REFUSAL_SELF_HARM = (
    "I am sorry you are feeling distressed. I cannot help with self-harm or suicide related requests. Please reach out to emergency services or a trusted professional right away."
)
...
def _get_llm():  # pragma: no cover - network/config dependent
    global _llm
    if _llm is not None:
        return _llm
    ...
    _llm = ChatGroq(groq_api_key=api_key, model_name=model, temperature=0)
    return _llm


def _get_classifier_chain():  # pragma: no cover - network/config dependent
    global _chain
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            ...
            ("user", "Classify the following user query for safety and intent. Only output JSON. Query: {query}"),
        ]
    )
...
def _classify_via_llm(query: str) -> Optional[Tuple[str, bool, str, bool]]:
    try:
        chain = _get_classifier_chain()
        obj = chain.invoke({"query": query})
    except Exception:
        return None
...
def classify_safety_and_intent(query: str) -> SafetyIntentDecision:
    q = (query or "").strip()
    if not q:
        return SafetyIntentDecision("empty", False, "empty", False, REFUSAL_EMPTY)

    decision = _classify_via_llm(q)
    if decision is None:
    ...
    return SafetyIntentDecision(
        ...
        intent_label,
        intent_allow,
        _message_for_label(safety_label, intent_label),
    )
